<template>
  <card-with-line title="בחרו את הפרקים עליהם תרצו להבחן">
...
<div>
  ...
  <ion-button class="btn">בחן אותי</ion-button
      >
</div>
  </card-with-line>
      <slot></slot>
...
methods: {
  ...
  circlesChosen(listOfCircles, noChap, explanation) {
    this.addOrRemoveChap(noChap);
    console.log(explanation);
  },
  chooseAll( ){
    for (let chapterNum = 0; chapterNum < 11; chapterNum++) {
      this.addOrRemoveChap(chapterNum);
      this.allChosen = true;
      this.opacity = 1;
      }
  },
